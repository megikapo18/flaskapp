...
class User:
    ...
    def get_all_user_info(cls, data):
        query= 'SELECT * FROM magazines WHERE magazines.user_id=%(user_id)s;'
    ...
    @classmethod
    def get_logged_user_subcribe_magazines(cls, data):
        query = 'SELECT magazine_id as id FROM subcribes LEFT JOIN users on subcribes.user_id = users.id WHERE user_id = %(user_id)s;'
        results= connectToMySQL(cls.db_name).query_db(query, data)
        subcribesmagazine = []
        for row in results:
            subcribesmagazine.append(row['id'])
        return subcribesmagazine

    @classmethod
    def update_user(cls,data):
        query='UPDATE users SET name=%(name)s, lastName =%(lastName)s, email = %(email)s WHERE users.id = %(user_id)s;'
        return connectToMySQL(cls.db_name).query_db(query,data)
